...
const read_items_by_item_code = async (account_id, item_code) => {
	const [result] = await db.query(`SELECT name, size, registered_date, purchase_cost, account_id FROM items WHERE account_id=? AND code=?`, [account_id, item_code]);
	//result[0] return undefined when there is no data with those conditions
	if(result == undefined){
		return undefined;
	}else{
...
module.exports = {
	...
	read_items_by_item_code,	//read by account_id & item_code
